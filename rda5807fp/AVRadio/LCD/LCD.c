...
void Send_byte(uint8_t);  //Передача данных LCD, вызывается функциями
                          // Send_command и Send_data.
...
{
    //Перед инициализацией LCD, надо в init задержку в 100 мсек сделать, чтоб
    //питание устаканилось.
    _delay_ms(100);
    ...
    Busy_flag();
    OutPin();
    DPORT |= (0 << DB7) | (0 << DB6) | (1 << DB5) |
             (0 << DB4);  // 4х битный интерфейс
    Strob();
...
void LCDnblank(
    void)  //Сделать видимой инфо на дисплее + отключить видимые курсоры.
{
    LCDcommand(0b1100);
}
...
void LCDresshift(void)  //Установить курсор в позицию 0,0 + сброс всех сдвигов,
...
//Системные функции, их не трогаем, они для работы дисплея.
void Send_byte(uint8_t i)  //Передача данных LCD, вызывается функциями
                           // Send_command и Send_data.
{
    OutPin();
